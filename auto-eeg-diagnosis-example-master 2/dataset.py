...
def wrapper_func(string_list,contents):
    index_list=[]
    for i in string_list:
        try:
            ans=contents.index(i)
            index_list.append(ans)
        except:
            pass
    if len(index_list)==0:
        index_list.append(len(contents))
    return min(index_list)

def return_text(path):
    in_file = open(path, "rt", errors="replace") 
    try:
        contents = in_file.read()         # read the entire file into a string variable
    except:
        pass
    index1=contents.find('IMPRESSION')+11
    index2=wrapper_func(['CLINICAL CORRELATION', 'CORRELATION', 'Clinical correlation', 'Clinical Correlation',         'correlation'],contents)
    wanted_text =" ".join(contents[index1:index2].split())
    in_file.close()
    return wanted_text
...
def load_data(fname, preproc_functions, sensor_types=['EEG']):
    ...
    assert set(cnt.ch_names)== set(selected_ch_names)
    n_sensors = 0
    if 'EEG' in sensor_types:
        n_sensors += 21
    if 'EKG' in sensor_types:
    ...
    for fn in preproc_functions:
        ...
        fs = float(fs)
    result=np.zeros((24,68286))
    result[:data.shape[0],:data.shape[1]] = data
    result=result.reshape((599,2736))
...
class DiagnosisSet(object):
    def __init__(self,all_file_names, n_recordings, max_recording_mins, preproc_functions,
                 data_folders,
    ...
    def load(self, only_return_labels=False):
        ...
        all_file_names=self.all_file_names
        ...
        for i_fname, fname in enumerate(cleaned_file_names[:self.n_recordings]):
            ...
            if '/abnormal/' in fname:
                ans='ssss '+return_text(fname[:-9]+'.txt')+' eeee'
                y_binary.append(1)
            elif '/normal/' in fname:
                ans='ssss '+ 'normal'+ ' eeee'
                y_binary.append(0)
            else:
                raise ValueError
